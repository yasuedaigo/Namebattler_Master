...
public class ConsoleManager {
    ...
    public void setPlayerToPlayerComponent(Party party) {
        ...
        for (int i = 0; i < GameConstants.NUMBER_OF_TEAM_MEMBERS; i++) {
            List<Player> playerList = members.stream().filter(p -> p.getTeam() == Teams.PLAYER).collect(Collectors.toList());
        ...
        for (int i = 0; i < GameConstants.NUMBER_OF_TEAM_MEMBERS; i++) {
            List<Player> enemyList = members.stream().filter(p -> p.getTeam() == Teams.ENEMY).collect(Collectors.toList());
